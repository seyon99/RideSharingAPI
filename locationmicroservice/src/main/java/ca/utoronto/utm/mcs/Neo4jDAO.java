...
public class Neo4jDAO {
    ...
    public Result nearestDrivers(double longitude, double latitude, int radius) {
        String query = "MATCH (n: user) " +
                "WHERE distance(point({ latitude: n.latitude, longitude: n.longitude}), point({ latitude: '%f', longitude: '%f'})) <= '%d' " +
                "RETURN n"; // or is it RETURN * (probably not)
        query = String.format(query, longitude, latitude, radius);
        return this.session.run(query);
    }

    public Result getBestRoute(String driverUid, String passengerUid) {
        // not sure if this works... may need to modify this method
        String query = "MATCH (n1:user {uid: '%s'}),\n" +
                "(n2:user {uid: '%s'}),\n" +
                "p = shortestPath((n1.street)-[:ROUTE_TO*]-(n2.street))\n" +
                "RETURN p";
